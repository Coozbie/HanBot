...
function Combo()
	...
	if target and common.IsValidTarget(target) then
		...
		if menu.combo.w:get() and target.pos:dist(player.pos) >= 470 then
			CastW(target)
		elseif menu.combo.w:get() and Isolated == true or not player:spellSlot(0).state == 0 then
			CastW(target)
		end
...
function qDmg(target)
	if Isolated == false then
		local qDamage = CalcADmg(target, QlvlDmg[player:spellSlot(0).level] + player.flatPhysicalDamageMod * 1.2, player)
		return qDamage
	else
		local qDamage = CalcADmg(target, (QlvlDmg[player:spellSlot(0).level] + player.flatPhysicalDamageMod * 1.2) * 0.65, player)
		return qDamage
	end
end
