...
local igniteDmg = { 80, 105, 130, 155, 180, 205, 230, 255, 280, 305, 330, 355, 380, 405, 430, 455, 480, 505} --"50 + (20 * myHero.level)"
...
local function AutoIgnite()
  	if menu.sum.ign.Ignite:get() and igniteSlot and player:spellSlot(igniteSlot).state == 0 then
    	for i = 0, objManager.enemies_n - 1 do
      	local enemy = objManager.enemies[i]
        	...
        	local damage = igniteDmg[player.levelRef]
        		if dist < (600 * 600) and dist >= (500 * 500) and damage > enemy.health then
          			player:castSpell("obj", igniteSlot, enemy)
          			break
        		end
...
local function OnDraw()
	...
	if smiteSlot and player:spellSlot(smiteSlot).state == 0 and menu.draws.dss:get() then
