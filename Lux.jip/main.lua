	...
	for i, enemy in pairs(GetEnemy()) do
		if not enemy.isDead and enemy.isVisible and enemy.isTargetable and menu.auto.uks:get() then
			...
			elseif player:spellslot(2).state == 0 and eDmg(enemy) > hp then
				CastE(enemy);
			elseif player:spellslot(3).state == 0 and player:spellslot(0).state == 0 and rDmg(enemy) + qDmg(enemy) > hp then
				CastQ(enemy)
				CastR(enemy)
			elseif player:spellslot(2).state == 0 and player:spellslot(3).state == 0 and rDmg(enemy) + eDmg(enemy) > hp then
				CastE(enemy)
				CastR(enemy)
			elseif player:spellslot(3).state == 0 and player:spellslot(0).state == 0 and player:spellslot(2).state == 0 and qDmg(enemy) + eDmg(enemy) + rDmg(enemy) > hp then
				CastE(enemy)
				CastQ(enemy)
				CastR(enemy)
			end
			if player:spellslot(3).state == 0 and rDmg(enemy) > hp and EnemyCC then
				CastR(enemy)
			end
		end
	end
